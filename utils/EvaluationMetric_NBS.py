...
def chamfer_weight(weight, gt):
    ...
    for j in range(weight.shape[1]):
        delta = weight[:, j][:, None] - gt #(N,J)
        delta = np.abs(delta).mean(axis=0) #(J,)
        res.append(np.min(delta, axis=0)) #float
    res = np.array(res)
    return res.mean()
...
def offset2bone(offset, parent):
    ...
    for i, p in enumerate(parent):
        if i == 0:
            continue
        res.append(torch.stack([joint[p], joint[i] - joint[p]], dim=0))#[2,3],由父节点坐标+当前节点的偏移表示bone
