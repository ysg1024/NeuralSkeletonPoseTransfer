# This file contains mainly SMPL code, which needs to be given parameters to generate SMPL models
#
# source：neural-blend-shapes/dataset/，The three pkl files including SMPL were also provided by NBS, which we converted to h5 files
...
class SMPLLayer(Module):
    def __init__(self, model_root='./smpl_model', gender='neutral'):
        super(SMPLLayer, self).__init__()

        if gender == 'neutral':
            self.model_path = os.path.join(model_root, 'smpl_basic_model_neutral.h5')
        elif gender == 'female':
            self.model_path = os.path.join(model_root, 'smpl_basic_model_f.h5')
        elif gender == 'male':
            self.model_path = os.path.join(model_root, 'smpl_basic_model_m.h5')
