# This file is mainly SMAL related code, you need to give the parameters to generate SMAL models
#
# Modified from the code of the smpl model, the pkl file of SMAL is downloaded from the official website, and we convert it into an h5 file
...
class SMALLayer(Module):
    def __init__(self, model_root='./smpl_model'):
        super(SMALLayer, self).__init__()


        self.model_path = os.path.join(model_root, 'smal.h5')


        dataH5 = h5py.File(self.model_path, "r")

        self.register_buffer('J_regressor', torch.from_numpy(dataH5['J_regressor'][()]).type(torch.float))
        self.register_buffer('joint_regressor', torch.from_numpy(dataH5['J_regressor'][()]).type(torch.float))
        self.register_buffer('weights', torch.from_numpy(dataH5['weights'][()]).type(torch.float))
        self.register_buffer('posedirs', torch.from_numpy(dataH5['posedirs'][()]).type(torch.float))
        self.register_buffer('v_template', torch.from_numpy(dataH5['v_template'][()]).type(torch.float))
        self.register_buffer('shapedirs', torch.tensor(dataH5['shapedirs'][()]).type(torch.float))
        self.kintree_table = dataH5['kintree_table'][()]
    ...
    def forward(self, pose, betas, trans=None, simplify=False):
        ...
        """
              ...
              Parameters:
              ---------
              pose: Also known as 'theta', an [N, 33, 3] tensor indicating child joint rotation
              relative to parent joint. For root joint it's global orientation.
              Represented in a axis-angle format.

              betas: Parameter for model shape. A tensor of shape [N, 41] as coefficients of
        ...
        results = stacked - \
                  self.pack(
                      torch.matmul(
                          stacked,
                          torch.reshape(
                              torch.cat((J, torch.zeros((batch_num, self.num_joints, 1), dtype=torch.float).to(device)),
                                        dim=2),
                              (batch_num, self.num_joints, 4, 1)
                          )
                      )
                  )
